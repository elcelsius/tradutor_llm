...
- Glossário: loader separa `source_aliases`/`aliases` de busca, `source_case_sensitive` para nomes técnicos ambíguos, `bad_aliases` com troca direta segura, `contextual_bad_aliases` reparados por LLM com concordância, `allowed_target_aliases` aceitos e `target_replacements` explícitos; o refine seleciona apenas formas PT-BR presentes no chunk; `--use-glossary` resolve `glossario/glossario_manual.json` ou `glossario/glossario_geral.json`.
