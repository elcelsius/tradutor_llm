...
- **Tradução**: `tradutor/translate.py`
  - Pré-processamento, chunking, glossário por chunk, chamadas LLM, sanitização.
  - Grava métricas/report e manifesto de progresso.
- **Desquebrar**: `tradutor/desquebrar.py`, `tradutor/desquebrar_safe.py`
  - Une linhas quebradas (LLM ou heurístico “safe”).
...
- **PDF**: `tradutor/pdf.py`, `tradutor/pdf_export.py`
  - Exporta Markdown → PDF usando ReportLab.
- **Utilitários**:
...
8. **PDF** (opcional): exporta `*_pt_refinado.md` → PDF.
