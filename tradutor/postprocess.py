...
def _normalise_straight_dialogue_quotes(text: str) -> str:
    ...
    replacements = {
        idx: "“" if order % 2 == 0 else "”" for order, idx in enumerate(positions)
    }
...
def final_pt_postprocess(text: str) -> str:
    ...
    replacements = [
        ...
        (
            r"\beu ter sido mandada para as Ruínas do Descarte\b",
            "eu ter sido mandado para as Ruínas do Descarte",
        ),
        ...
        (
            r"\bSeu socorro de primeiros socorros e no combate\b",
            "Sua ajuda nos primeiros socorros e no combate",
        ),
        (
            r"\bSeu primeiro socorro e sua ajuda na luta\b",
            "Sua ajuda nos primeiros socorros e na luta",
        ),
        (
            r"\bSeu primeiro socorro e na luta\b",
            "Sua ajuda nos primeiros socorros e na luta",
        ),
        (
            r"\bSeu primeiro socorro e no combate\b",
            "Sua ajuda nos primeiros socorros e no combate",
        ),
        (
            r"\bSeu socorro de emergência e (na luta|no combate)\b",
            r"Sua ajuda nos primeiros socorros e \1",
        ),
        (
            r"\bSeu socorro e no combate\b",
            "Sua ajuda nos primeiros socorros e no combate",
        ),
        (
            r"\bFoi sua muitas estratégias que decidiu a (batalha|vitória)\b",
            r"Foram as suas muitas estratégias que decidiram a \1",
        ),
        (
            r"\bFoi sua muitas estratégias que venceu a batalha\b",
            "Foram as suas muitas estratégias que decidiram a batalha",
        ),
        (
            r"\bTouka, você foi afiado como uma lâmina\b",
            "Touka, você foi muito esperto",
        ),
        (
            r"\bColocar sua vida em jogo pode ter sido um pouco demais, embora\.",
            "Colocar sua vida em jogo pode ter sido um pouco demais.",
        ),
        (
            r"\bArriscar sua vida talvez tenha sido um pouco demais, embora\.",
            "Arriscar sua vida talvez tenha sido um pouco demais.",
        ),
        (
            r"\bverificar se minhas criações funcionando corretamente\b",
            "verificar se minhas criações estão funcionando corretamente",
        ),
        ...
        (
            r"\bseres temporários para servir a um propósito\b",
            "identidades temporárias adequadas a um propósito",
        ),
        ...
        (
            r"\bexerce em (seu|sua|o|a) corpo para considerar\b",
            r"exerce sobre \1 corpo e que precisamos considerar",
        ),
        (
            r"\bexerce sobre (seu|sua|o|a) corpo para considerar\b",
            r"exerce sobre \1 corpo, algo que precisamos considerar",
        ),
        (
            r"\bexerce sobre (seu|sua|o|a) corpo e que precisamos considerar\b",
            r"exerce sobre \1 corpo, algo que precisamos considerar",
        ),
        (
            r"\bexerce no corpo dela para considerar\b",
            "exerce sobre o corpo dela, algo que precisamos considerar",
        ),
        (
            r"\bimpõe ao corpo (dele|dela) para considerar\b",
            r"impõe ao corpo \1, algo que precisamos considerar",
        ),
        (r"\buma pano\b", "um pano"),
        (r"\bessa foi um teste\b", "isso foi um teste"),
        (
            r"\bmais ou menos igual ao de Paralisar\b",
            "mais ou menos igual ao da habilidade Paralisar",
        ),
        (
            r"\bmais ou menos igual ao do Paralisar\b",
            "mais ou menos igual ao da habilidade Paralisar",
        ),
        (
            r"\bmais ou menos o mesmo que Paralisar\b",
            "mais ou menos igual ao da habilidade Paralisar",
        ),
        (
            r"\bAliás, eu nem sei (.*?), agora que parou pra pensar\b",
            r"Aliás, eu nem sei \1, agora que parei para pensar",
        ),
        (
            r"\bHavia limites no que podíamos alcançar por meio daquele familiar\b",
            "Havia limites para o que podíamos descobrir por meio daquele familiar",
        ),
        (
            r"\bHavia limites ao que conseguíamos através daquele familiar\b",
            "Havia limites para o que conseguíamos descobrir por meio daquele familiar",
        ),
        ...
        (
            r"\bidentidades temporárias para um propósito\b",
            "identidades temporárias adequadas a um propósito",
        ),
        (
            r"\bAcredito que o verdadeiro eu seja\b",
            "Acredito que o verdadeiro você seja",
        ),
        (
            r"\bo apanhador no campo de centeio dela\b",
            "seu apanhador no campo de centeio",
        ),
        (r"\bser lavada cerebralmente\b", "sofrer lavagem cerebral"),
        (
            r"\bem frente o ([A-ZÀ-Ý][A-Za-zÀ-ÿ'-]*(?:\s+[A-ZÀ-Ý][A-Za-zÀ-ÿ'-]*){0,3})\b",
            r"em frente ao \1",
        ),
        (r"\bnosso carroça\b", "nossa carruagem"),
        (
            r"\bYasu fora um dos vários que haviam pegado no caminho\b",
            "Yasu fora um dos vários que eles haviam levado consigo pelo caminho",
        ),
        (
            r"\bTodos emergiram em segurança do outro lado da luta\b",
            "Todos saíram ilesos do confronto",
        ),
        (r"\bthey todos\b", "todos"),
        (
            r"\bSer atencioso… é nisso que a Hijiri culpa pelo seu fracasso\b",
            "Ser atencioso… é isso que Hijiri culpa pelo próprio fracasso",
        ),
        (
            r"\bGentileza não é o tipo de coisa que você pode rejeitar de mão beijada\b",
            "Gentileza não é o tipo de coisa que dá pra simplesmente rejeitar assim, de cara",
        ),
        (
            r"\baumentar seu poder como deusa, talvez\?",
            "aumentar seu poder como divindade, talvez?",
        ),
        (
            r"\bquando o Congelar for removido\b",
            "quando o efeito de Congelar for removido",
        ),
        (r"\bconvenceu a me apoiar\b", "convenceu a confiar em mim"),
        (
            r"\bMas se enviar nós, heróis, para casa consome\b",
        ...
        (
            r"\bguardou tudo através das Ruínas do Descarte\b",
            "guardou tudo durante a passagem pelas Ruínas do Descarte",
        ),
        (r"\bNossa avó do lado da nossa mãe\b", "Nossa avó materna"),
        (
            r"\bYasu lembrava do punho fechado que o dono da carruagem fez ao dizer aquelas palavras\b",
            "Yasu se lembrou de como o dono da carruagem fechou o punho ao dizer aquelas palavras",
        ),
        (r"\bmensageiros de montaria rápidos\b", "mensageiros a cavalo velozes"),
        (
            r"\bA gente evitava essas criaturas justamente por isso\b",
            "A gente evitava a estrada justamente por causa dessas criaturas",
        ),
        (r"\bDeixe o de fora pra gente\b", "Deixe o que está lá fora por nossa conta"),
        (
            r"\bArriscar sua vida pode ter sido um pouco demais, embora\.",
            "Arriscar sua vida pode ter sido um pouco demais.",
        ),
        (r"\bArright\b", "Beleza"),
        (r"\bboost\b", "impulso"),
        ...
        (
            r"\bTudo (?:o )?que você serve é para se ajoelhar\b",
            "Você só serve para se ajoelhar",
        ),
        ...
        (
            r"\beu faço um ótimo Deusazinha\b",
            "eu faço uma ótima imitação da Deusazinha",
        ),
        (r"\bthough\b", "porém"),
        (
            r"\bSeras-san está aqui, vou poupá-lo de usá-lo como exemplo\b",
            "Seras-san está aqui, vou poupá-la de usá-lo como exemplo",
        ),
        (
            r"\ba Seras-san está aqui então vou poupá-lo de usar ele como exemplo\b",
            "a Seras-san está aqui, então vou poupá-la de usá-lo como exemplo",
        ),
        (
            r"\ba Seras-san está aqui então vou poupá-lo de usar como exemplo\b",
            "a Seras-san está aqui, então vou poupá-la de usá-lo como exemplo",
        ),
        (
            r"\ba Seras-san está aqui então vou poupá-lo de usá-lo como exemplo\b",
            "a Seras-san está aqui, então vou poupá-la de usá-lo como exemplo",
        ),
        (
            r"\bcomo a Seras Ashrain está aqui, vou poupá-lo de usar ele como exemplo\b",
            "como a Seras Ashrain está aqui, vou poupá-la de usá-lo como exemplo",
        ),
        (
            r"\bEla viu a espada da Seras\s*—\s*está se cuidando dela\b",
            "Ela viu a espada da Seras — está tomando cuidado com ela",
        ),
        ...
        (
            r"\bTem cuidado de manter-se fora do alcance de seus ataques\b",
            "Toma cuidado para se manter fora do alcance de seus ataques",
        ),
        (
            r"\bpara apoiar a perna de Seras e fornecendo suspensão\b",
            "para apoiar a parte inferior do corpo de Seras e dar sustentação",
        ),
        (
            r"\bPodíamos agora tomar ações intensas montados sem sermos derrubados do chão\b",
            "Agora podíamos realizar manobras intensas montados sem sermos derrubados",
        ),
        (
            r"\bEle viu a espada da Seras\s*—\s*está se mantendo fora do alcance dela\.\s*Cuidadoso para não chegar perto demais de seus ataques\.",
            "Ele viu a espada da Seras e está se precavendo contra ela. Toma cuidado para ficar fora do alcance de seus ataques.",
        ),
        (
            r"\bEle viu a espada da Seras\s*—\s*está cauteloso com ela\b",
            "Ele viu a espada da Seras — está se precavendo contra ela",
        ),
        (
            r"\bEle viu a espada da Seras\s*—\s*está desconfiado dela\.\s*Cuidadoso para manter uma distância segura de seus ataques\.",
            "Ele viu a espada da Seras e está se precavendo contra ela. Toma cuidado para ficar fora do alcance de seus ataques.",
        ),
        (
            r"\balterando sua forma para apoiar a parte inferior do corpo de Seras e fornecendo suspensão\b",
            "alterando sua forma para apoiar a parte inferior do corpo de Seras e dar sustentação",
        ),
        (
            r"\balterando sua forma para apoiar o corpo inferior da Seras e fornecendo suspensão\b",
            "alterando sua forma para apoiar a parte inferior do corpo de Seras e dar sustentação",
        ),
        (
            r"\bAgora podíamos tomar ações intensas a cavalo sem sermos derrubados\b",
            "Agora podíamos realizar manobras intensas a cavalo sem sermos derrubados",
        ),
        (
            r"\bAgora podíamos tomar ações intensas (?:montados|a cavalo) sem sermos (?:derrubados|lançados) ao chão\b",
            "Agora podíamos realizar manobras intensas a cavalo sem sermos derrubados",
        ),
        (r"\bsem sermos derrubados do chão\b", "sem sermos derrubados"),
        (
            r"\bV-vocês, divindades, podem ser interessantes então, não é\?",
            "V-vocês, divindades, até que podem ser interessantes, não são?",
        ),
        (r"\bforça nos números\b", "força na quantidade"),
        (
            r"\bhumanos que você estava prestes a gritar seu amor por eles\b",
            "humanos por quem você estava prestes a gritar seu amor",
        ),
        (
            r"\bTragarei o maior sofrimento sobre toda dimensão e todo mundo\b",
    ...
    cleaned = re.sub(
        r"\bVocês ganham neles nesse aspecto\b",
        "Vocês levam vantagem nesse aspecto",
        cleaned,
    )
    cleaned = re.sub(
        r"\bvocês ganham neles nesse aspecto\b",
    ...
    cleaned = re.sub(
        r"\bAh,\s+você Pode calar a boca\b", "Ah, você pode calar a boca", cleaned
    )
    ...
    for ln in cleaned.splitlines():
        ...
        if prev_nonempty and not is_dialog and not prev_dialog:
            final_lines.append(
                ""
            )  # insere linha vazia entre parágrafos narrativos consecutivos
