...
def write_model_output(out_dir: Path, slug: str, model: str, translated: str, elapsed: float, input_path: Path) -> str:
    ...
    out_path.write_text("
".join(header) + translated, encoding="utf-8")
    return out_path.name


def write_summary(out_dir: Path, slug: str, input_path: Path, used_chars: int, endpoint: str, rows: list[tuple[str, str, float]]) -> None:
    lines = [
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", default="benchmarks", help="Diretório de saída para resultados")
    parser.add_argument(
        "--endpoint",
