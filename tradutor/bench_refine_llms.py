...
def call_ollama_pipeline(
    ...
    output_path = (
        Path(cfg.output_dir) / f"{input_path.stem}_{model_slug}_refine_body.md"
    )
    start = time.monotonic()
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark de refine com varios modelos Ollama."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num-ctx", type=int, help="Override de refine_num_ctx do config.yaml"
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if args.use_glossary:
        ...
        dynamic_path_override = (
            Path(args.dynamic_glossary) if args.dynamic_glossary else None
        )
    ...
    if args.max_chars > 0:
        effective_input_path = (
            state_dir / f"{input_path.stem}_input_{args.max_chars}.md"
        )
    ...
    for model in models:
        ...
        try:
            ...
            if args.single_prompt:
                ...
                if model_glossary_state:
                    glossary_block = format_glossary_for_prompt(
                        model_glossary_state.combined_index
                    )
            ...
            fname = write_model_output(
                out_dir, slug, model, refined, elapsed, input_path
            )
