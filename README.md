...
- Tempos: `saida/<slug>_timings.json` contém o run mais recente, enquanto `saida/timings/<slug>_<timestamp>_<status>.json` preserva cada execução. Com `--debug`, outra cópia fica em `debug_runs/<slug>/<timestamp>/99_reports/timings.json`. O tempo de `translate` inclui o repair; repair e revisão bilíngue também aparecem em `nested_stages.translation_repair` e `nested_stages.bilingual_review`, sem dupla contagem.
