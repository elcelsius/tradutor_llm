...
class _AmputatingRepairBackend:
    ...
    def generate(self, prompt: str):
        ...
        text = (
            "### TEXTO_REPARADO_INICIO\n"
            "“Nyaki… Nyaki—me desculpe… Me desculpe por ter demorado tanto pra te encontrar…!”\n"
            "### TEXTO_REPARADO_FIM"
        )
        return type("Resp", (), {"text": text})
...
def test_translate_document_repairs_chunk_before_final_output(tmp_path: Path) -> None:
    ...
    result = translate_document(
        pdf_text='"I have no desire to die," replied Seras calmly, choosing not to answer directly.',
        backend=backend,
        cfg=cfg,
...
def test_repair_rejects_candidate_that_removes_existing_translation(
    ...
    source = (
        ...
        "“I have no desire to die,” replied Seras calmly."
    )
    translated = (
        "“Deixei isso a seu critério, certo? Se você decidiu vir, tudo bem pra mim.”\n\n"
        "O rosto da Nyaki se iluminou. “Nyaki está tão feliz de ver você de novo, mestre!”\n\n"
        "O Piggymaru e o Slei realmente adoram a Nyaki.\n\n"
        "“I have no desire to die,” replied Seras calmly, choosing not to answer directly."
    )
    result = repair_translation_chunk(
        source_text=source,
