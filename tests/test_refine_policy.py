...
def test_refine_is_opt_in_after_translation() -> None:
    ...
    assert _should_run_refine_after_translate(
        SimpleNamespace(no_refine=False, refine=True), cfg
    )
    assert not _should_run_refine_after_translate(
        SimpleNamespace(no_refine=True, refine=True), cfg
    )
